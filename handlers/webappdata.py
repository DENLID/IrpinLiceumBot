...
async def wad_handler(message: Message, db: MDB):
    ...
    if (
        f'{data["class_number"]}-{data["class_letter"]}'
        in (await db.users.find_one({"_id": int(message.chat.id)}))["tags"]
    ):
...
Відсутні: {data["ms"]}
""",
            reply_markup=keyboards.ms_confirm_kb(
                data["class_letter"],
                int(data["class_number"]),
                data["students_number"],
                int(data["students_number"]) - int(data["ms_number"]),
                data["ms_number_hv"],
                data["ms"],
            ),
        )
        ...
        await message.answer(
            f'Вибачте але ви не можете редагувати відсутніх в класі {data["class_number"]} - {data["class_letter"]}',
            reply_markup=keyboards.to_comm_kb,
        )
