...
@router.errors(or_f(ExceptionTypeFilter(AttributeError), ExceptionTypeFilter(ValueError)), F.update.message.as_("message"))
async def error_handler(event: ErrorEvent, message: Message):
    if str(event.exception) == "'NoneType' object has no attribute 'split'" or "invalid literal for int() with base 10" in str(event.exception):
